...
export class ArtskartModel {
  constructor() {
    ...
    autorun(() => {
      ...
      if (h)
        h.SpeciesCountCalculated = ArtskartModel.gangeDerTomMultiplierBetyr1(
          h.SpeciesCount,
          h.SpeciesCountDarkFigure
        );
    });
    autorun(() => {
      const h = this.expandedSpreadHistory;
      if (h) {
        //                if (!h.ExistenceArea) {
        h.ExistenceAreaCalculated = ArtskartModel.gangeDerTomMultiplierBetyr1(
          h.ExistenceArea,
          h.ExistenceAreaCountDarkFigure
        );
        h.ExistenceAreaCountCalculated =
          ArtskartModel.gangeDerTomMultiplierBetyr1(
            h.ExistenceArea,
            h.ExistenceAreaCountDarkFigure
          );
        h.SpreadAreaCalculated = ArtskartModel.gangeDerTomMultiplierBetyr1(
          h.SpreadArea,
  ...
  hentFulltFylkeNavn(kortnavn) {
    if (!this.koder) return kortnavn;
    if (!this.koder.countyListLand) return kortnavn;
    for (let i = 0; i < this.koder.countyListLand.length; i++) {
      const referenceId = this.koder.countyListLand[i].Value;
      if (referenceId === kortnavn) return this.koder.countyListLand[i].Text;
    }
  }
  hentKortFylkeNavn(fylkenavn) {
    if (!this.koder) return fylkenavn;
    if (!this.koder.countyListLand) return fylkenavn;
    for (let i = 0; i < this.koder.countyListLand.length; i++) {
      const referenceId = this.koder.countyListLand[i].Text;
      if (referenceId === fylkenavn) return this.koder.countyListLand[i].Value;
    }
  }

  regionListe() {
    return this.koder.countyListLand.map((r) => {
  ...
  addSpreadHistory(vurdering) {
    ...
    if (!vurdering.SpreadHistory) vurdering.SpreadHistory = [];
    vurdering.SpreadHistory.push(ny);
  ...
  expandSpreadHistory(item) {
    ...
    if (item.visArtskart === undefined) item.visArtskart = false;
  ...
  enhanceRegionalPresence(regionalPresenceString) {
    if (regionalPresenceString === null) regionalPresenceString = "";
    ...
    const regs = this.koder.countyListLand.reduce((acc, r) => {
  ...
  mapSpreadHistoryToUI(spreadHistory) {
    for (let i = 0; i < spreadHistory.length; i++) {
      ...
      if (!v.SelectionGeometry) v.SelectionGeometry = "";

      v.regionalPresenceKnown = this.enhanceRegionalPresence(v.Regions);
      v.regionalPresenceAssumed = this.enhanceRegionalPresence(
        v.RegionsAssumed
      );
    }
  }
